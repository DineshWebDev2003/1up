...
import { View, Text, StyleSheet, TouchableOpacity, SafeAreaView, ScrollView, Switch, Modal, TextInput } from 'react-native';
...
const SettingsScreen = () => {
  ...
  const helpContent = (
    <Text style={styles.modalText}>
      For any support or queries, please contact us at support@tnhappykids.com or call us at +91 12345 67890.
    </Text>
  );
                  ...
                  <TouchableOpacity key={index} style={[styles.optionButton, index === section.options.length - 1 && styles.noBorder]} onPress={option.type !== 'switch' ? option.action : null} activeOpacity={option.type === 'switch' ? 1 : 0.7}>
                    ...
                    {option.type === 'switch' ? (
                      <Switch
                        ...
                        thumbColor={option.value ? Colors.white : '#f4f3f4'}
                        onValueChange={option.action}
                        value={option.value}
                      />
                    ) : (
                      <Ionicons name="chevron-forward-outline" size={20} color={'rgba(255, 255, 255, 0.7)'} />
                    )}
                  </TouchableOpacity>
                ))}
              </View>
