...
const TeacherChatsScreen = () => {
  ...
  const fetchChatList = async () => {
    try {
      ...
      if (chatResult.success) {
        const formattedData = chatResult.data.map(user => ({...user, lastMessage: 'Tap to start a conversation', timestamp: ''}));
  ...
  const handleCallPress = (user) => {
    // Note: You'll need a way to get parent/guardian numbers for students.
    // This is a placeholder until that data is available in the API.
    if (user.role === 'Student') {
        Alert.alert('Contact Info', 'Contact details for students are not yet available.');
        return;
    }
  ...
  const renderConversationItem = ({ item }) => {
    ...
    return (
      <Animatable.View animation="fadeInUp" duration={600} delay={100} style={styles.chatItemWrapper}>
        <Link href={`/(common)/chat-detail?userId=${item.id}&name=${encodeURIComponent(item.display_name || item.name)}&avatar=${encodeURIComponent(item.avatar || '')}`} asChild>
                ...
                {selectedUserForCall.contacts && Object.keys(selectedUserForCall.contacts).length > 0 ? (
                  Object.entries(selectedUserForCall.contacts).map(([type, info]) => (
                    ...
                    >
                      ...
                      <View>
                        <Text style={styles.contactText}>{info.name || 'Unknown'}</Text>
                        <Text style={styles.contactNumber}>{type}: {info.number || 'N/A'}</Text>
                      </View>
                    </TouchableOpacity>
                  ))
                ) : (
