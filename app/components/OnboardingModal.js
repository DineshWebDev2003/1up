...
const OnboardingModal = ({ isVisible, onClose, studentData, onProfileUpdate }) => {
  ...
  const handleSave = async () => {
    // First save location data
    const locationSaved = await saveLocationData();
    if (!locationSaved) {
      return; // Don't proceed if location save failed
    }
    ...
    formData.append('mother_number', motherNumber || '');
    formData.append('guardian_name', guardianName || '');
    formData.append('guardian_number', guardianNumber || '');
