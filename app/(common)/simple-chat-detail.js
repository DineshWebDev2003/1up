...
export default function SimpleChatDetail() {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      const response = await authFetch(`/api/messages/get_messages.php?user_id1=${senderId}&user_id2=${receiverId}`);
  ...
  const sendMessage = async (messageData = null) => {
    ...
    try {
      ...
      const response = await authFetch('/api/chat/send_simple_message.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleImagePicker = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        // For now, just send the image name as a message
        const imageMessage = {
          sender_id: senderId,
          receiver_id: receiverId,
          message: `📷 Image: ${result.assets[0].fileName || 'image.jpg'}`,
          message_type: 'text'
        };
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.attachmentModal}>
            ...
            >
              <MaterialIcons name="insert-drive-file" size={24} color={Colors.textSecondary} />
              <Text style={[styles.attachmentText, { color: Colors.textSecondary }]}>Document (Coming Soon)</Text>
            </TouchableOpacity>
