...
export default function NewAttendanceScreen() {
  ...
  const [attendanceRecords, setAttendanceRecords] = useState([]); // detailed list for bottom section
  ...
  const fetchStudents = useCallback(async (currentBranchId) => {
    ...
    try {
      ...
      // 2) Fetch new-attendance records for selected date/branch and merge
      try {
  ...
  // Fetch today's new-attendance records for bottom detailed list
  const fetchNewAttendance = useCallback(async () => {
    try {
      const formattedDate = date.toISOString().split('T')[0];
      const branchQuery = selectedBranchId && selectedBranchId !== 'All' ? `&branch_id=${selectedBranchId}` : '';
      const res = await authFetch(`/api/attendance/get_new_attendance.php?date=${formattedDate}${branchQuery}`);
  ...
  const handleGuardianSelection = async (guardianType, guardianName) => {
    try {
      ...
      if (result.success) {
        ...
        // Append/refresh bottom detailed list
        await fetchNewAttendance();
      ...
      <FlatList
        ...
        ListFooterComponent={() => (
          <View style={styles.detailedSection}>
            <Text style={styles.detailedTitle}>Today's Marks</Text>
            {attendanceRecords.length === 0 ? (
              <Text style={styles.detailedEmpty}>No marks yet.</Text>
            ) : (
              attendanceRecords.map((r) => (
                <View key={`${r.id}`} style={styles.detailedCard}>
                  <View style={styles.detailedRowTop}>
                    <Text style={styles.detailedName}>{r.student_name || r.username || r.student_code}</Text>
                    <Text style={[styles.detailedStatus, r.status === 'present' ? styles.statusPresent : styles.statusAbsent]}>
                      {r.status}
                    </Text>
                  </View>
                  <View style={styles.detailedRow}>
                    <Text style={styles.detailedLabel}>In:</Text>
                    <Text style={styles.detailedValue}>{r.in_time || '--:--'}</Text>
                    <Text style={styles.detailedLabel}>By:</Text>
                    <Text style={styles.detailedValue}>{
                      (r.in_guardian_type || r.in_guardian_name)
                        ? `${r.in_guardian_type || ''}${r.in_guardian_type && r.in_guardian_name ? ' ' : ''}${r.in_guardian_name || ''}`.trim()
                        : '--'
                    }</Text>
                  </View>
                  <View style={styles.detailedRow}>
                    <Text style={styles.detailedLabel}>Out:</Text>
                    <Text style={styles.detailedValue}>{r.out_time || '--:--'}</Text>
                    <Text style={styles.detailedLabel}>By:</Text>
                    <Text style={styles.detailedValue}>{
                      (r.out_guardian_type || r.out_guardian_name)
                        ? `${r.out_guardian_type || ''}${r.out_guardian_type && r.out_guardian_name ? ' ' : ''}${r.out_guardian_name || ''}`.trim()
                        : '--'
                    }</Text>
                  </View>
                  {(r.father_name || r.mother_name || r.guardian_name) && (
                    <View style={styles.detailedRowWrap}>
                      {r.father_name ? <Text style={styles.relationBadge}>Father: {r.father_name}</Text> : null}
                      {r.mother_name ? <Text style={styles.relationBadge}>Mother: {r.mother_name}</Text> : null}
                      {r.guardian_name ? <Text style={styles.relationBadge}>Guardian: {r.guardian_name}</Text> : null}
                    </View>
                  )}
                </View>
              ))
...
const styles = StyleSheet.create({
  ...
  detailedSection: { marginTop: 20, paddingHorizontal: 15, paddingBottom: 30 },
  detailedTitle: { fontSize: 18, fontWeight: 'bold', color: Colors.lightText, marginBottom: 10 },
  detailedEmpty: { color: Colors.textSecondary, fontSize: 14 },
  detailedCard: {
    backgroundColor: Colors.card,
    borderRadius: 14,
    padding: 12,
  ...
  detailedRowTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 },
  detailedName: { fontSize: 16, fontWeight: '700', color: Colors.darkText },
  detailedStatus: { fontSize: 12, fontWeight: '700', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8, color: Colors.lightText },
  statusPresent: { backgroundColor: '#16a34a' },
  statusAbsent: { backgroundColor: '#dc2626' },
  detailedRow: { flexDirection: 'row', alignItems: 'center', marginTop: 4 },
  detailedRowWrap: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 8 },
  detailedLabel: { fontSize: 12, color: Colors.textSecondary, marginRight: 6 },
  detailedValue: { fontSize: 13, color: Colors.darkText, marginRight: 12, fontWeight: '600' },
  relationBadge: { fontSize: 12, color: Colors.lightText, backgroundColor: Colors.primary, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 10 },
  modalContainer: {
    flex: 1,
