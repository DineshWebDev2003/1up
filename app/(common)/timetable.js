    ...
    try {
      ...
      if (result.success) {
        ...
        if (!numericInitialBranchId && result.data.length > 0) {
          setSelectedBranch(parseInt(result.data[0].id, 10));
            ...
            <View style={styles.pickerContainer}>
              <Picker
                ...
                onValueChange={(itemValue) => {
                  ...
                  console.log('Branch selected:', numericId);
              ...
              >
                {(branches && Array.isArray(branches)) ? branches.map((b) => <Picker.Item key={b.id} label={b.name} value={parseInt(b.id, 10)} />) : null}
              </Picker>
            </View>
          </Animatable.View>
        )}
