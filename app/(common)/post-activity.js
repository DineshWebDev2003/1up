...
const PostActivityScreen = () => {
  ...
  const handleChooseMedia = async (mediaType) => {
    ...
    if (mediaType === 'image' && pickerResult.assets && pickerResult.assets.length > 0) {
      ...
      selectedAsset = { ...pickerResult.assets[0], uri: manipResult.uri, type: 'image', width: pickerResult.assets[0].width, height: pickerResult.assets[0].height };
...
const styles = StyleSheet.create({
  ...
  imagePreviewContainer: { marginBottom: 15, alignItems: 'center', width: '100%' },
  imagePreview: { width: '100%', height: 250, borderRadius: 10, backgroundColor: '#f5f5f5' },
