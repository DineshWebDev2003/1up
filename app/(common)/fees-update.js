...
export default function FeesUpdateScreen() {
  ...
  const fetchStudentData = useCallback(async (currentUser) => {
    ...
    try {
      ...
      // Use new student_fees API for fetching fees data
      let feesUrl = '/api/fees/student_fees_update.php';
      ...
      const feesMap = (feesResult.data && Array.isArray(feesResult.data)) ? feesResult.data.reduce((acc, fee) => {
          // Map by student_id from the new student_fees table
        ...
        const combinedData = (usersResult.data && Array.isArray(usersResult.data)) ? usersResult.data.map(student => {
          const feeDetails = feesMap[student.id] || { total_fees: '0.00', amount_paid: '0.00', pending_amount: '0.00' };
          ...
          return {
            ...
            status: status,
            fee_type: feeDetails.fee_type || 'monthly',
            due_date: feeDetails.due_date || null
          };
        }) : [];
  ...
  const handleUpdate = async (student) => {
    try {
      ...
      if (dueDate) {
        ...
        // Provide a default due date if none exists (30 days from now)
        const defaultDate = new Date();
        defaultDate.setDate(defaultDate.getDate() + 30);
        dueDate = defaultDate.toISOString().split('T')[0]; // Format as YYYY-MM-DD
      }

      const response = await authFetch('/api/fees/student_fees_update.php', {
        method: 'PUT',
        headers: {
                    ...
                    <View style={styles.editContainer}>
                      ...
                      <View style={styles.inputGroup}>
                        ...
                        <TextInput
                          style={styles.feeInput}
                          value={item.due_date || ''}
                          onChangeText={(text) => handleFeesChange(item.id, 'due_date', text)}
