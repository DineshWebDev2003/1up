...
export default function AssignNewUserScreen() {
  ...
  const [franchiseeShare, setFranchiseeShare] = useState('75');
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.data && data.data.length > 0) {
          ...
          setSelectedClassName(data.data[0].name);
        }
      } else {
  ...
  const handleAssignUser = async () => {
    ...
    // Get current user role to determine approval flow
    ...
    let userStatus = 'inactive'; // Default to inactive (pending approval)
    if (currentUser.role === 'Admin') {
      userStatus = 'active'; // Only Admin creates active users directly
    }
    
    console.log('User will be created with status:', userStatus);
    
    const userData = {
      name: name, // API expects 'name' field
      username: name, // Also send username for compatibility
      branch_id: selectedBranch,
      role,
      mobile,
      email,
      password,
      franchisee_share: role === 'Franchisee' ? parseFloat(franchiseeShare) : null,
      number_plate: role === 'Captain' ? numberPlate : undefined,
      class_id: role === 'Student' ? selectedClass : undefined,
      fees: (role === 'Student' || role === 'Tuition Student') && studentFees ? parseFloat(studentFees) : undefined,
      status: userStatus, // Primary status field
      user_status: userStatus, // Backup status field
      approval_status: userStatus, // Approval status field
      created_by: currentUser.role || 'Unknown',
      created_by_role: currentUser.role || 'Unknown',
      // IMPORTANT: Ensure dual storage for students
      store_in_users_table: true,
      // Create student entry for Student and Tuition Student roles
      create_student_entry: role === 'Student' || role === 'Tuition Student',
      // Additional fields for users table
      phone: mobile, // Ensure phone field is set
      // For students, add class information (send class name, not ID)
      class_name: (role === 'Student' || role === 'Tuition Student') && selectedClassName ? selectedClassName : undefined,
    };
      ...
      if (result.success) {
        // Store created user data for popup
        setCreatedUserData({
          name: name,
          email: email,
          role: role,
          studentId: result.data?.student_id || null,
          isStudent: role === 'Student' || role === 'Tuition Student',
          isActive: result.data?.status === 'active',
          isPending: result.data?.status === 'inactive',
          status: result.data?.status || 'active',
          createdBy: result.data?.created_by || currentUser.role
        });
    ...
    <SafeAreaView style={styles.safeArea}>
        ...
        <Animatable.View animation="fadeInUp" duration={800} delay={200} style={styles.formContainer}>
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={styles.inputGroup}>
                ...
                {/* Admin can always select any branch, others see their branch if pre-selected */}
              ...
              {role === 'Franchisee' && (
                <View style={styles.inputWrapper}>
                  ...
                  <TextInput 
                    ...
                    style={styles.input} 
                    value={franchiseeShare} 
                    onChangeText={setFranchiseeShare} 
              ...
              {role === 'Student' && (
                <>
                    ...
                    <View style={styles.pickerContainer}>
                      <Picker
                        ...
                        onValueChange={(itemValue) => {
                          ...
                          const selectedClassObj = classes.find(c => c.id.toString() === itemValue);
                          if (selectedClassObj) {
                  ...
                  <View style={styles.inputGroup}>
                    ...
                    <View style={styles.inputWrapper}>
                      <Ionicons name="cash-outline" size={22} style={styles.inputIcon} />
                      <TextInput 
                        style={styles.input} 
                        placeholder="Enter monthly fees amount" 
                        value={studentFees} 
                        onChangeText={setStudentFees} 
                        keyboardType="numeric" 
                        placeholderTextColor={Colors.gray} 
                      />
                    </View>
      ...
      {createdUserData && (
        <SuccessPopup
          ...
          studentId={createdUserData.studentId}
          isStudent={createdUserData.isStudent}
          isPending={createdUserData.isPending}
          status={createdUserData.status}
          onViewIdCard={() => {
            console.log('Navigate to ID card for student:', createdUserData.studentId);
            setShowSuccessPopup(false);
            setCreatedUserData(null);
            // TODO: Add navigation to ID card screen
          }}
          onCreateAnother={() => {
            setShowSuccessPopup(false);
            setCreatedUserData(null);
