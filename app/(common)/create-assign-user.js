...
export default function AssignUserScreen() {
  ...
  const handleAssignUser = async () => {
    ...
    const userData = {
      ...
      password: password,
      phone: mobile,
      // Ensure dual storage for students
      create_student_entry: selectedRole === 'Student' || selectedRole === 'Tuition Student',
      // Send class name instead of class_id
      class_name: (selectedRole === 'Student' || selectedRole === 'Tuition Student') ? studentClass : undefined,
    };

    try {
      const response = await fetch('/api/users/user_crud.php', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        ...
        if (result.data && result.data.student_id) {
          ...
          successMessage += `\n💾 Stored in both Users and Students tables`;
        }
        successMessage += `\n✨ Status: ${result.data?.status === 'inactive' ? 'Waiting for Approval' : 'Active'}`;
        
        // Show animated success popup with options
        Alert.alert(
          '🎉 User Created!', 
          successMessage,
          [
            {
              ...
              onPress: () => {
                if (result.data && result.data.student_id) {
                  console.log('Navigating to ID card for student:', result.data.student_id);
                }
              },
              style: 'default'
            },
            {
              text: 'Create Another',
              onPress: () => {
                console.log('Ready to create another user');
              },
              style: 'default'
            },
            {
  ...
  const renderRoleSpecificFields = () => {
    ...
    return (
      <Animatable.View animation="fadeInUp">
        ...
        {(selectedRole === 'Student' || selectedRole === 'Tuition Student') && <DropdownField icon="school-outline" placeholder="Select Class" items={studentClasses} onValueChange={setStudentClass} value={studentClass} />}
      </Animatable.View>
    );
  };
