...
export default function FeesScreen() {
  ...
  const loadFeesData = async (isRefresh = false) => {
    try {
      ...
      const response = await authFetch(`/api/fees/get_student_fees.php?student_id=${studentId}`);
  ...
  const generateQRCode = async () => {
    if (!feesData?.fees_details?.pending_amount || feesData.fees_details.pending_amount <= 0) {
    ...
    try {
      const response = await authFetch('/api/fees/generate_qr_code.php', {
        ...
        body: JSON.stringify({
          ...
          amount: feesData.fees_details.pending_amount
        })
      });
  ...
  const payWithRazorpay = async () => {
    ...
    try {
      const amountRupees = Number(feesData.fees_details.pending_amount) || 0;
  ...
  const uploadPaymentScreenshot = async () => {
    ...
    try {
      ...
      formData.append('amount', String(feesData?.fees_details?.pending_amount || '0'));
  ...
  const total = feesData?.fees_details?.total_amount ?? 0;
  const pending = feesData?.fees_details?.pending_amount ?? 0;
...
const styles = StyleSheet.create({
  ...
  cardsRow: { flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 15, marginTop: -15 },
  cardWrap: { flex: 1, marginHorizontal: 5 },
  ...
  actionButtons: { flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 20, marginTop: 30 },
  qrButton: { flex: 1, marginRight: 10 },
  uploadButton: { flex: 1, marginLeft: 10 },
