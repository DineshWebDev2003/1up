...
const ChatDetailScreen = () => {
  const { name, userId, id: chatPartnerId, chatPartnerId: altChatPartnerId, avatar, chatId } = useLocalSearchParams();
  ...
  useEffect(() => {
    fetchMessages();
    const interval = setInterval(fetchMessages, 10000); // Poll every 10 seconds instead of 5
    return () => clearInterval(interval);
            ...
            <Image 
              source={
                (chatPartner?.avatar && chatPartner.avatar.trim() !== '') 
                  ...
                  : (chatPartner?.profile_image && chatPartner.profile_image.trim() !== '')
                    ? { uri: chatPartner.profile_image.startsWith('http') ? chatPartner.profile_image : `${API_URL}${chatPartner.profile_image}` }
