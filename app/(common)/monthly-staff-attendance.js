...
const MonthlyStaffAttendance = ({ staff, date, onBack }) => {
  ...
  const handleDayPress = (staffName, day, attendanceData) => {
    setSelectedDayData({ staffName, day, ...attendanceData });
  ...
  const renderDay = ({ item: dayData, staffMember }) => {
    ...
    return (
      <TouchableOpacity style={styles.dayCell} onPress={() => handleDayPress(staffMember.name, dayData.day, attendanceData)}>
        <Ionicons 
  ...
  const renderStaffRow = ({ item }) => (
    <View style={styles.staffRow}>
      <View style={styles.staffNameCell}>
        <Text style={styles.staffName}>{item.name}</Text>
        <Text style={styles.staffId}>(Staff ID: {item.id})</Text>
      </View>
      <FlatList
        data={daysArray}
        renderItem={({ item: dayData }) => renderDay({ item: dayData, staffMember: item })}
  ...
  return (
    <View style={styles.container}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <Animatable.View animation="fadeIn" duration={800} delay={400}>
              <View style={styles.headerRow}>
                <View style={[styles.staffNameCell, styles.headerCell]}><Text style={styles.headerCellText}>Staff</Text></View>
      ...
      <Modal animationType="zoomIn" transparent={true} visible={modalVisible} onRequestClose={() => setModalVisible(!modalVisible)}>
          ...
          <Animatable.View animation="zoomIn" duration={400} style={styles.modalView}>
            ...
            {selectedDayData && (
              <View style={styles.modalContent}>
                <Text style={styles.modalText}><Text style={styles.modalLabel}>Staff:</Text> {selectedDayData.staffName}</Text>
...
const styles = StyleSheet.create({
  ...
  staffRow: { flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: Colors.border, backgroundColor: Colors.white },
  staffNameCell: { width: 150, padding: 12, justifyContent: 'center', backgroundColor: Colors.lightGray },
  staffName: { fontSize: 14, fontWeight: 'bold', color: Colors.darkGray },
  staffId: { fontSize: 12, color: Colors.gray },
