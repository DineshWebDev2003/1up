...
import { View, Text, StyleSheet, FlatList, Image, Dimensions, TouchableOpacity, Modal, Animated, SafeAreaView, Platform, TouchableWithoutFeedback, Alert } from 'react-native';
...
const KidsFeedScreen = () => {
  ...
  useEffect(() => {
    const fetchFeed = async () => {
      try {
        const url = branch_id ? `/api/activities/get_activities.php?branch_id=${branch_id}` : '/api/activities/get_activities.php';
        ...
        if (result.success) {
          ...
          const transformedData = result.data.map(activity => ({
            ...
            activityName: activity.title || activity.description,
    ...
    <SafeAreaView style={styles.safeArea}>
      <Animatable.View animation="fadeInDown" duration={800}>
        <LinearGradient colors={Colors.gradientMain} style={styles.header}>
          ...
          <Text style={styles.headerSubtitle}>{branch ? branch : 'All Branches'}</Text>
        </LinearGradient>
      </Animatable.View>
...
const styles = StyleSheet.create({
  ...
  headerSubtitle: { fontSize: 16, color: Colors.white, opacity: 0.9, textAlign: 'center', marginTop: 4 },
